...
class TransientNonlinearSolver(object):
    ...
    def __init__(self, rho, kappa, velocity, f, g, bcs, *args, **kwargs):
        super(TransientNonlinearSolver, self).__init__(*args, **kwargs)
        ...
        print EC.getAbsTemp(E_.vector().array(), p_air)
        E_prev = E_
        ...
        while t <= t_e:
            ...
            E_mid = 0.5*(E+E_prev)
            F = (rho*(E-E_prev)*v*dx + dt*(inner(kappa*nabla_grad(E_mid), nabla_grad(v))*dx 
                + rho*inner(velocity*nabla_grad(E_mid), nabla_grad(v))*dx 
                + f*v*dx - g*v*ds(2)))
...
bcs = [DirichletBC(V, Constant(E_surf), boundary_parts, 1)]
...
time_control = dict(t_a=t_a, t_e=t_e, dt=dt)
E_surf = Expression('E_0 + delta_E_0*sin(2*pi/period*t)', E_0=E_0, delta_E_0=delta_E_0, period=period, t=t_a)
E_base = EC.getEnth(T_base, 0., p_air)
bcs = [E_surf, E_base]
...
if do_verfication:
    ...
    transient_problem = DirichletBCTransientNonlinearSolver(kappa_cold(E), velocity, f, g, bcs, time_control=time_control)
    E_sol = transient_problem.E_sol
